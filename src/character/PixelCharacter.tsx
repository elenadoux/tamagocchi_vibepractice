...
export function PixelCharacter({playing,sleeping,hasPlayed,mode='PET',pet='cat'}:{pet?:PetId;playing:boolean;sleeping:boolean;hasPlayed:boolean;mode?:VisualMode}){
 ...
 useEffect(()=>{const ctx=canvas.current!.getContext('2d')!;ctx.imageSmoothingEnabled=false;let frames=createCharacterFrames(pet);let disposed=false;if(pet==='frog')void loadFrogFrames().then(loaded=>{if(!disposed)frames=loaded}).catch(()=>{});let raf=0,last=0,frame=0;const memory:ReactionMemory={state:'idle',until:0,lastBeat:0,lastExcited:0};const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
 ...
 if(pet!=='frog'&&state==='sleep'){ctx.font='10px monospace';ctx.fillText('z',91,26);ctx.fillText('Z',97,14-(frame%2)*2)}else if(pet!=='frog'&&p.playing){const shift=frame%3;ctx.fillRect(99,15+shift,2,8);ctx.fillRect(101,15+shift,4,2);ctx.fillRect(96,22+shift,3,2);ctx.fillRect(23,39-shift,2,7);ctx.fillRect(25,39-shift,3,2);ctx.fillRect(21,44-shift,3,2);if(frame%2===0){ctx.fillRect(19,20,5,1);ctx.fillRect(21,18,1,5);ctx.fillRect(106,45,5,1);ctx.fillRect(108,43,1,5)}}
