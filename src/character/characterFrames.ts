...
export function createCharacterFrames(pet:PetId='cat'){
 ...
 for(const state of states)result[state]=Array.from({length:4},(_,frame)=>{
  ...
  ellipse(11,27,pet==='frog'?5:3,2);ellipse(21,27,pet==='frog'?5:3,2);
  const lift=active?(frame%2===0?-2:2):1;
  if(pet==='cat'){
   polygon([[8,15],[7,5],[14,11]]);polygon([[18,11],[25,5],[24,16]]);
   ellipse(26,23,3,2);ellipse(28,20+frame%2,1,4);
  }else if(pet==='dog'){
  ...
  if(pet!=='bird'){ellipse(7,21+lift,2,2);ellipse(25,21-lift,2,2)}
  ...
  const eyeY=pet==='frog'?12:18,blink=state==='sleep'||(!active&&frame===3);
  ...
  if(pet==='cat'){pixel(7,21,3);pixel(23,21,3);pixel(9,10,1,3);pixel(23,10,1,3)}
