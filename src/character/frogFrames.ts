import sourceUrl from '../assets/frog-states-source.png';
...
const regions:Record<CharacterState,[number,number,number,number]>={
 idle:[25,195,325,280],playing:[372,146,350,325],dance:[744,145,325,330],jump:[1100,116,310,360],excited:[110,583,366,341],paused:[552,653,341,273],sleep:[958,577,413,349]
};
export type FrogFrames=Record<CharacterState,HTMLCanvasElement[]>;
let loading:Promise<FrogFrames>|undefined;
export function loadFrogFrames():Promise<FrogFrames>{
 if(loading)return loading;
 loading=new Promise<HTMLImageElement>((resolve,reject)=>{const image=new Image();image.onload=()=>resolve(image);image.onerror=()=>reject(new Error('Could not load frog artwork'));image.src=sourceUrl}).then(image=>{
  const result={} as FrogFrames;
  for(const [state,rect] of Object.entries(regions) as [CharacterState,[number,number,number,number]][]){
   ...
   // to the outside. Enclosed white eye highlights and all frog colors survive.
   const pixels=ctx.getImageData(0,0,w,h),data=pixels.data,seen=new Uint8Array(w*h),queue:number[]=[];
   const add=(i:number)=>{if(seen[i])return;seen[i]=1;const k=i*4,r=data[k],g=data[k+1],b=data[k+2];if(Math.min(r,g,b)>115&&Math.max(r,g,b)-Math.min(r,g,b)<40)queue.push(i)};
   ...
   // Keep the belly, cheeks, eye highlights and dark outline readable in four tones.
   const palette=[[32,42,28],[104,125,82],[157,178,131],[187,201,160]];
   for(let k=0;k<data.length;k+=4){if(data[k+3]===0)continue;const luminance=.2126*data[k]+.7152*data[k+1]+.0722*data[k+2];const tone=palette[luminance<100?0:luminance<190?1:luminance<225?2:3];data[k]=tone[0];data[k+1]=tone[1];data[k+2]=tone[2]}
   ...
   // The renderer enlarges this grid exactly 2×, never 72→64 fractional scaling.
